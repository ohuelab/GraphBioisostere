...
if os.path.exists(os.path.join(args.target_dir, "dataset.pt")):
...
def convert_records(df):
    ...
    unique_smiles = set(df['REF-FRAG'].tolist() + df['PRB-FRAG'].tolist())
    for smi in tqdm(unique_smiles, desc="Processing unique SMILES"):
        try:
            ...
            try:
                smiles_to_data_dict[smi] = smiles_to_data("C"+smi) # 1原子だけの場合は処理できない
                smiles_to_data_dict[smi].smiles = smi  # SMILES を Data に埋め込む
            except Exception as e:
...
torch.save(X,
    os.path.join(args.target_dir, "dataset.pt")
)
