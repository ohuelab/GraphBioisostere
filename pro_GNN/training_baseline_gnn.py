...
# ベースラインエンコーダーをインポート
from encoder.baseline_gnn_encoder import GINEncoder, GCNEncoder, GINEncoderWithEdge
...
def load_baseline_encoder(args, encoder_type='gin'):
    """
    ベースラインエンコーダーをロード
    
    Args:
        args: 設定引数
        encoder_type: 'gin', 'gcn', 'gin_edge' のいずれか
    """
    pooling = getattr(args, 'pooling', 'sum')
    
    if encoder_type == 'gin':
        encoder = GINEncoder(
            node_in=args.node_in,
            edge_in=args.edge_in,  # 使用しないが互換性のため
    ...
    elif encoder_type == 'gin_edge':
        encoder = GINEncoderWithEdge(
            node_in=args.node_in,
...
class Trainer:
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, encoder_type='gin'):
        self.args = args
        self.local_rank = int(os.environ["LOCAL_RANK"])
        self.encoder_type = encoder_type  # ベースラインエンコーダータイプを保存
        ...
        if self.local_rank == 0:  # マスターランクのみ保存
            ...
            args_dict['encoder_type'] = encoder_type  # エンコーダータイプも保存
        ...
        if self.local_rank == 0:
            self.log_message("=" * 50)
            self.log_message("Baseline GNN Training:")
            self.log_message(f"  Encoder type: {encoder_type}")
            ...
            self.log_message(f"  Pooling: {getattr(args, 'pooling', 'sum')}")
        ...
        # モデルとオプティマイザの初期化（ベースラインエンコーダーを使用）
        encoder = load_baseline_encoder(args, encoder_type=encoder_type).to(self.device)
    ...
    def save_checkpoint(self, epoch):
        ...
        if self.local_rank == 0:  # マスターランクのみ保存
            checkpoint = {
                ...
                'encoder_type': self.encoder_type  # エンコーダータイプも保存
            }
    ...
    def test(self):
        ...
        self.log_message(f"encoder_type={self.encoder_type}")  # エンコーダータイプもログ出力
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Baseline GNN Classifier Training')
    ...
    # Model
    parser.add_argument('--encoder_type', type=str, default='gin', choices=['gin', 'gcn', 'gin_edge'],
                        help='Baseline encoder type')
    ...
    parser.add_argument('--pooling', type=str, default='sum', choices=['sum', 'mean'],
                        help='Global pooling method for baseline encoders')
...
def main(args):
    ...
    # エンコーダータイプの取得
    encoder_type = getattr(args, 'encoder_type', 'gin')
    ...
    # Trainer 実行（encoder_typeを渡す）
    trainer = Trainer(args, train_loader, val_loader, test_loader, encoder_type=encoder_type)
