...
parser.add_argument("target_dir", type=str, default="unimols/")
args = parser.parse_args()

if os.path.exists(os.path.join(args.target_dir, "dataset_reg.pt")):
    print(f"Regression dataset already exists in {args.target_dir}")
    exit()

data = pd.read_csv(f"{args.target_dir}/dataset.csv", index_col=0)
...
# データの変換関数（回帰用）
def convert_records_regression(df):
    records = []
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Creating pairs"):
        try:
            smi1 = row['REF-SMILES']
            smi2 = row['PRB-SMILES']
            delta_value = row['delta_value']  # 回帰用：連続値を使用
            ...
            if smi1 in smiles_to_data_dict and smi2 in smiles_to_data_dict:
                ...
                pair = {
                    'data1': data1,
                    'data2': data2,
                    'label': torch.tensor([delta_value], dtype=torch.float),  # 回帰用：連続値
                }
                records.append(pair)
            else:
...
records = convert_records_regression(data)

X = MoleculePairDataset(records)
os.makedirs(args.target_dir, exist_ok=True)

torch.save(X, os.path.join(args.target_dir, "dataset_reg.pt"))